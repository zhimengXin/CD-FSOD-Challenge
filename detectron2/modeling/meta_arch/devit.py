...
# class BNINMix2d(nn.Module):
#     def __init__(self, num_features, beta_a=0.01, beta_b=0.01):
#         super().__init__()
#         assert beta_a > 0 and beta_b > 0

#         self.bn = nn.BatchNorm2d(num_features)
#         self.inorm = nn.InstanceNorm2d(num_features, affine=True)

#         init_delta = beta_a / (beta_a + beta_b)   # 0.5 when a=b=0.01
#         self.delta = nn.Parameter(torch.tensor(float(init_delta)))

#     def forward(self, x):
#         delta = torch.clamp(self.delta, 0.0, 1.0)
#         bn_out = self.bn(x)
#         in_out = self.inorm(x)
#         return (1.0 - delta) * bn_out + delta * in_out
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.training or self.use_one_shot:
            with torch.no_grad():
                ...
                gt_boxes = [x.gt_boxes.tensor for x in gt_instances]  #真实框

                rpn_boxes = [x.proposal_boxes.tensor for x in proposals]  # 候选框
                ...
                for proposals_per_image, targets_per_image in zip(boxes, gt_instances):
                    ...
                    matched_idxs, matched_labels = self.proposal_matcher(match_quality_matrix) # 将 候选框 和 真实框 进行匹配，得到匹配的框索引和标签。
        ...
        roi_features = self.roi_align(patch_tokens, rois)  # N, C, k,   # 使用 ROI Align 提取特征其中 N 是候选框的数量，C 是特征图的通道数，k 是每个 ROI 区域的空间大小
        roi_bs = len(roi_features) # 计算 ROI 特征的数量，roi_bs 是候选框的总数
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            feats = roi_features.transpose(-2, -1) @ class_weights.T  # 根据每个候选框的特征和类别权重来计算每个候选框属于各个类别的得分
            ...
            intra_dist_emb = intra_dist_emb.permute(0, 2, 3, 1).flatten(0, 1).reshape(bs * num_active_classes, -1,
                                                                                      self.roialign_size,
                                                                                      self.roialign_size)
            # Top-K 类别采样 和 类别间特征的计算。模型会根据得分选择 Top-K 类别，然后处理与目标类别相关的特征，同时计算其他类别的特征
            if self.set_cd_vito:
            ...
            # (Nxclasses) x 1
            # per_cls_input = self.cls_mix_norm(per_cls_input)
            cls_logits = self.per_cls_cnn(per_cls_input)

            # 将目标类别的特征（intra_dist_emb）、其他类别的特征（inter_dist_emb）和背景特征（bg_dist_emb_c）拼接在一起，形成一个新的输入张量
        ...
        if self.training:
            ...
            if self.use_mask:  # 提供更精细的 object shape 帮助 detection
                loss_dict.update(
